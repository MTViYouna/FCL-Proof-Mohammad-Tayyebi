...
try:
    ...
    if res.returncode != 0:
        print(f"[!] iperf3 failed explicitly. Stderr: {res.stderr}")
        sys.exit(1)
    ...
    metrics = {
        "Port": PORT, "GTL_Enabled": FCL_ACTIVE, "GTL_Granted": gtl_granted,
        "TokenWait_s": round(wait_time, 2), "TransferTime_s": round(transfer_time, 2),
        "Throughput_Mbps": round(bps, 2), "Retransmissions": total_retr
    }
    with open(METRICS_PATH, "w") as f: json.dump(metrics, f)

finally:
    if FCL_ACTIVE and gtl_granted:
        try: sock.sendto(json.dumps({"action": "release"}).encode("utf-8"), (GTL_IP, 5000))
        except: pass
    sock.close()
